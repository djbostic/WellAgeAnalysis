...
wellanalysis <- function(x=dwws){
  cgwl_at_dw <- raster::extract(cgwl_raster, setslist[[1]]) # intersect to get value of current water level at well points
  cad_dw <- cbind(setslist[[1]], cgwl_at_dw) 
  ...
  print(length(unique(activedw$WCRNUMBER)) / length(unique(cad_dw$WCRNUMBER))) # percent of wells whose TCD is below current groundwater levels (useable wells)
  ...
  mad$dry <- ifelse((mad$mt_at_dw >= mad$TOTALCOMPLETEDDEPTH & mad$TOTALCOMPLETEDDEPTH > 0), "TCDdry",
                    ifelse((mad$top > 0 & mad$mt_at_dw >= mad$top & mad$mt_at_dw < mad$TOTALCOMPLETEDDEPTH), "topdry",
